...
class DoubleEliminationViewController: UIViewController, UITextFieldDelegate, UIScrollViewDelegate {
    ...
    func reloadStackViewBracket() {
        ...
        while (i >= 0) {
            ...
            if i == 15 {
                ...
                let upperView = self.largeSubView.viewWithTag(219)!
                let xVal = upperView.center.x - 20
                let imgVw = UIImageView(frame: CGRect(x: xVal, y: yVal, width: 40, height: 40))
                imgVw.image = UIImage(named: "downArrow.png")
                self.largeSubView.addSubview(imgVw)
                ...
                let upperView = self.largeSubView.viewWithTag(217)!
                let xVal = upperView.center.x - 20
                let imgVw = UIImageView(frame: CGRect(x: xVal, y: yVal, width: 40, height: 40))
                imgVw.image = UIImage(named: "downArrow.png")
                self.largeSubView.addSubview(imgVw)
    ...
    func updateLaterMatchWithWinner(winnerId: String?, idxOfPrevMatch: Int) {
        if winnerId == nil {
            ...
            if winnerId == GlobalConstants.tie {
                CoreDataUtil.clearMatch(self.bracketMatches[1], clearLeft: false, clearRight: true)
                CoreDataUtil.clearMatch(self.bracketMatches[0], clearLeft: false, clearRight: true)
                return
            }
            var nextMatch = self.bracketMatches[1]
